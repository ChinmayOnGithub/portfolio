...
export function About({
  ...
  return (
    ...
    >
      ...
      {/* Return button when Skills index is open */}
      {showSkills && (
        <button 
          onClick={onToggleSkills}
          className="absolute top-3 right-3 z-30 inline-flex items-center gap-1 text-[10px] uppercase font-bold tracking-widest text-[var(--accent-color)] hover:opacity-80 transition-all font-sans focus:outline-none focus-visible:ring-1 focus-visible:ring-[var(--accent-color)]"
      ...
      <CardHeader className="pb-2 relative z-10">
        <CardTitle className="text-xl font-bold font-cormorant text-[var(--text-color)]">About Me</CardTitle>
      </CardHeader>
      <CardContent className="pt-0 pb-4 relative z-10 pr-12">
        <p className="text-base leading-relaxed font-times text-[var(--text-color)]/90">
